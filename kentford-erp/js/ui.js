...
const UI={
 ...
 modal(o){
  ...
  el.innerHTML=`<div class="dlg ${o.wide?'wide':''}"><div class="dlg-h"><b>${esc(o.title||'')}</b><button class="btn-x" data-act="modal-close" title="${esc(t('common.close'))}">×</button></div><div class="dlg-b">${o.body||''}</div>${o.foot?`<div class="dlg-f">${o.foot}</div>`:''}</div>`;
 ...
 ask({title,fields=[],vals={},ok,wide,pre='',post='',danger}){
  ok=ok||t('common.save');
  return new Promise(res=>{
   const m=this.modal({title,wide,body:pre+Form.render(fields,vals)+post,
    foot:`<button class="btn btn-o" data-x="c">${esc(t('common.cancel'))}</button><button class="btn ${danger?'btn-d':''}" data-x="ok">${esc(ok)}</button>`});
 ...
 confirm({title,msg,reason=false,ok,danger=false}){
  return this.ask({title:title||t('common.confirm'),ok:ok||t('common.yes_continue'),danger,pre:`<p>${msg}</p>`,fields:reason?[{k:'reason',l:t('common.reason'),t:'textarea',req:true}]:[]});
 },
 ...
 timeline(col,id){
  const ev=[...DB.col('audit').filter(a=>a.col===col&&a.ref===id).map(a=>({at:a.at,by:a.userName,t:`<b>${esc(a.action)}</b>${a.reason?': '+esc(a.reason):''}`})),
   ...DB.all('comments').filter(c=>c.refCol===col&&c.refId===id).map(c=>({at:c.createdAt,by:c.byName,t:esc(t('common.comment'))+': '+esc(c.text)}))]
   .sort((a,b)=>b.at.localeCompare(a.at));
  return ev.length?`<div class="tl">${ev.map(e=>`<div class="ev">${e.t}<br><small>${esc(e.by)} • ${fdt(e.at)}</small></div>`).join('')}</div>`:`<div class="empty">${esc(t('common.no_activity'))}</div>`;
 },
 activity(col,id){
  return `<div class="activity" data-col="${col}" data-id="${id}"><textarea rows="2" placeholder="${esc(t('common.comment_placeholder'))}"></textarea>
   <div class="right" style="margin:6px 0 12px"><button class="btn btn-sm" data-act="comment-add">${esc(t('common.send_comment'))}</button></div><div class="tl-box">${this.timeline(col,id)}</div></div>`;
 }
};
ACT['modal-close']=el=>el.closest('.ov')._m.close();
ACT['comment-add']=el=>{
 const w=el.closest('.activity'),ta=$('textarea',w),text=ta.value.trim();
 if(!text)return UI.toast(t('common.comment_empty'),'err');
 const {col,id}=w.dataset;
 DB.insert('comments',{refCol:col,refId:id,text,byName:Auth.user.name});
 (text.match(/@([\w.]+)/g)||[]).forEach(m=>{const u=DB.all('users').find(x=>x.username.toLowerCase()===m.slice(1).toLowerCase());if(u)Notify.user(u.id,t('common.mention_notify',{name:Auth.user.name,text:text.slice(0,80)}),location.hash)});
 ta.value='';$('.tl-box',w).innerHTML=UI.timeline(col,id);App.refreshBell();
};
...
const Form={
 ...
 select(name,f,val,dis,extra=''){
  const o=this.opts(f);
  return `<select ${name} ${dis} ${extra}><option value="">— ${esc(t('common.select'))} —</option>${o.map(x=>`<option value="${esc(x.v)}" ${String(x.v)===String(val??'')?'selected':''}>${esc(x.l)}</option>`).join('')}</select>`;
 },
 field(f,val,ro){
...
class DT{
 ...
 draw(){
  ...
  $('.dt-body',this.el).innerHTML=`<div class="tblw"><table><thead><tr>${cols.map(x=>`<th class="${x.num?'num':''} ${x.sort===false?'':'sortable'}" ${x.sort===false?'':`data-act="dt-sort" data-k="${esc(x.k)}" data-dt="${this.id}"`}>${esc(x.l)}${this.sort&&this.sort.k===x.k?(this.sort.d>0?' ▲':' ▼'):''}</th>`).join('')}</tr></thead>
   <tbody>${sl.map(r=>`<tr class="${c.onRow?'clk':''}" ${c.onRow?`data-act="dt-row" data-dt="${this.id}" data-id="${r.id}"`:''}>${cols.map(x=>`<td class="${x.num?'num':''}">${x.html?x.html(r):esc(this.val(x,r)??'')}</td>`).join('')}</tr>`).join('')||`<tr><td colspan="${cols.length}" class="empty">${esc(c.empty||t('common.no_data'))}</td></tr>`}</tbody></table></div>
   <div class="pager"><span class="mut">${rows.length} ${esc(t('common.data_count_suffix'))}</span><span><button class="btn btn-o btn-sm" data-act="dt-page" data-d="-1" data-dt="${this.id}" ${this.page<=1?'disabled':''}>‹ ${esc(t('common.prev'))}</button> ${esc(t('common.page'))} ${this.page}/${pages} <button class="btn btn-o btn-sm" data-act="dt-page" data-d="1" data-dt="${this.id}" ${this.page>=pages?'disabled':''}>${esc(t('common.next'))} ›</button></span></div>`;
 }
...
ACT['dt-xls']=el=>{const dt=DT.inst[el.dataset.dt],x=dt.export();Export.xls(dt.cfg.title||'data',x.head,x.rows)};
ACT['dt-print']=el=>{const dt=DT.inst[el.dataset.dt],x=dt.export();
 Print.html(Print.header(dt.cfg.title||t('common.data_list'),fdate(today()))+`<table><tr>${x.head.map(h=>`<th>${esc(h)}</th>`).join('')}</tr>${x.rows.map(r=>`<tr>${r.map(c=>`<td>${esc(c)}</td>`).join('')}</tr>`).join('')}</table>`)};
...
const App={closed:new Set(),
 ...
 refreshBell(){
  ...
  const p=$('#pn');if(p)p.innerHTML=`<div class="it"><b>${esc(t('common.notifications'))}</b> <a href="#" data-act="notif-read" style="float:right">${esc(t('common.mark_all_read'))}</a></div>`+
   (Notify.mine().slice(0,15).map(x=>`<a class="it ${x.read?'':'un'}" href="#" data-act="notif-open" data-id="${x.id}">${esc(x.text)}<br><small class="mut">${fdt(x.createdAt)}</small></a>`).join('')||`<div class="it mut">${esc(t('common.no_notifications'))}</div>`);
  this.badges();
 },
 profile(){
  $('#pp').innerHTML=`<div class="it"><b>${esc(Auth.user.name)}</b><br><small class="mut">${esc(Auth.role?.name||'')} • ${esc(Auth.user.username)}</small></div>
   <a class="it" href="#" data-act="chpw">${esc(t('common.change_password'))}</a><a class="it" href="#" data-act="logout">${esc(t('common.logout'))}</a>`;
 },
 search(q){
  ...
  box.innerHTML=out.join('')||`<div class="empty">${esc(t('common.no_results'))}</div>`;box.classList.remove('hide');
 }
};
