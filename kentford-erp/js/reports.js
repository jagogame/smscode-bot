...
const REPORTS=[
 ...
 // ---------- L. Laporan Aftersales Partner Network ----------
 // Beberapa laporan memakai satu render dasar (kolom partner umum) dengan filter status berbeda —
 // hanya "partner_coverage" & "partner_perf"/"partner_cost" yang benar-benar bespoke kolomnya.
 {key:'partner_list',label:t('reports.partner_list'),need:()=>can('partners','r'),f:['status'],st:()=>PARTNER_STATUS_OPTS,
  rows:f=>Partners.all().filter(r=>!f.status||r.status===f.status),
  cols:[R_COL('code','Kode',r=>r.code),R_COL('name',t('common.customer'),r=>r.name),R_COL('pic','PIC',r=>r.pic),R_COL('prov',t('partner.province'),r=>r.province),R_COL('city','Kota',r=>r.city),
   R_COL('st',t('common.status'),r=>r.status),R_COL('rating',t('partner.rating'),r=>String(num(r.rating)||'-'),{num:true})]},
 {key:'partner_prospect_list',label:t('reports.partner_prospect_list'),need:()=>can('partner_prospects','r'),f:[],
  rows:()=>DB.all('partner_prospects'),
  cols:[R_COL('name',t('common.customer'),r=>r.name),R_COL('prov',t('partner.province'),r=>r.province),R_COL('city','Kota',r=>r.city),R_COL('st',t('common.status'),r=>r.status),
   R_COL('next',t('reports.col_next'),r=>fdate(r.nextFollowupDate)),R_COL('conv','Dikonversi',r=>r.converted?'Ya':'Tidak')]},
 {key:'partner_by_island',label:t('reports.partner_by_island'),need:()=>can('partners','r'),f:[],
  rows:()=>{const m={};Partners.all().forEach(r=>{const isl=(PartnerCode.lookup(r.province)||['LAINNYA'])[0];m[isl]=(m[isl]||[]);m[isl].push(r)});
   return Object.entries(m).map(([island,rows])=>({island,n:rows.length,active:rows.filter(r=>r.status==='Active').length}))},
  cols:[R_COL('island','Pulau',r=>r.island),R_COL('n','Jumlah Partner',r=>String(r.n),{num:true}),R_COL('active',t('partner.col_active'),r=>String(r.active),{num:true})]},
 {key:'partner_by_province',label:t('reports.partner_by_province'),need:()=>can('partners','r'),f:[],
  rows:()=>Partners.provinceSummary(),
  cols:[R_COL('p',t('partner.province'),r=>r.province),R_COL('a',t('partner.col_active'),r=>String(r.active),{num:true}),R_COL('pr',t('partner.col_prospect'),r=>String(r.prospect),{num:true}),
   R_COL('i',t('partner.col_inactive'),r=>String(r.inactive),{num:true}),R_COL('n',t('partner.col_nearest'),r=>r.nearest?r.nearest.name+' ('+r.nearest.distKm+' km)':'-')]},
 {key:'partner_coverage_gap',label:t('reports.partner_coverage_gap'),need:()=>can('partners','r'),f:[],
  rows:()=>[...new Set(DB.all('customers').map(c=>c.city))].filter(Boolean).map(p=>Partners.regionPriority(p)),
  cols:[R_COL('p',t('partner.province'),r=>r.province),R_COL('c',t('partner.col_customers'),r=>String(r.custCount),{num:true}),R_COL('d',t('partner.col_nearest'),r=>r.nearestName?r.nearestName+' ('+r.distKm+' km)':'-'),
   R_COL('lvl',t('partner.col_priority'),r=>r.level)]},
 {key:'partner_coverage_detail',label:t('reports.partner_coverage_detail'),need:()=>can('partners','r'),f:[],
  rows:()=>Partners.all().flatMap(r=>(r.coverageAreas||[]).map(a=>({...a,partner:r.name,code:r.code}))),
  cols:[R_COL('code','Kode',r=>r.code),R_COL('name',t('common.customer'),r=>r.partner),R_COL('area','Wilayah',r=>r.area||'-'),R_COL('cost','Biaya Transport',r=>rp(r.transportCost||0),{num:true}),R_COL('eta','Estimasi (jam)',r=>String(r.etaHours||'-'))]},
 {key:'partner_job_history',label:t('reports.partner_job_history'),need:()=>can('partners','r'),f:[],
  rows:()=>DB.all('svc_req').filter(r=>r.partnerId),
  cols:[R_COL('no','No. Service',r=>r.no),R_COL('p','Partner',r=>r.partnerName||'-'),R_COL('c',t('common.customer'),r=>custName(r.customerId)),R_COL('st',t('common.status'),r=>r.status)]},
 {key:'partner_perf',label:t('reports.partner_perf'),need:()=>can('partner_evaluations','r'),f:[],
  rows:()=>Partners.all().map(r=>({...r,...Partners.stats(r.id)})),
  cols:[R_COL('code','Kode',r=>r.code),R_COL('name',t('common.customer'),r=>r.name),R_COL('rating',t('partner.rating'),r=>String(r.avgRating||'-'),{num:true}),
   R_COL('jobs',t('partner.jobs'),r=>String(r.completedCount),{num:true}),R_COL('evals','Jml Evaluasi',r=>String(r.evalCount),{num:true})]},
 {key:'partner_cost',label:t('reports.partner_cost'),need:()=>can('partner_payments','r'),f:[],
  rows:()=>DB.all('partner_payments'),
  cols:[R_COL('p','Partner',r=>DB.get('partners',r.partnerId)?.name||'-'),R_COL('fee','Biaya Jasa',r=>rp(r.serviceFee),{num:true}),R_COL('trans','Transport',r=>rp(r.transportCost),{num:true}),
   R_COL('tot','Total',r=>rp(r.total),{num:true,sortv:r=>r.total})],
  sum:rows=>t('reports.data_count',{n:rows.length})+' • Total: '+rp(sum(rows,r=>r.total))},
 {key:'partner_payments',label:t('reports.partner_payments_rep'),need:()=>can('partner_payments','r'),f:['status'],st:()=>['Menunggu Approval','Siap Dibayar','Dibayar'],
  rows:f=>DB.all('partner_payments').filter(r=>!f.status||r.paymentStatus===f.status),
  cols:[R_COL('p','Partner',r=>DB.get('partners',r.partnerId)?.name||'-'),R_COL('tot','Total',r=>rp(r.total),{num:true}),R_COL('st',t('common.status'),r=>r.paymentStatus),R_COL('d','Tanggal Bayar',r=>fdate(r.paymentDate))]},
 {key:'partner_contract_expiry',label:t('reports.partner_contract_expiry'),need:()=>can('partners','r'),f:[],
  rows:()=>Partners.all().filter(r=>r.contractEndDate&&r.contractEndDate<=addDays(today(),60)),
  cols:[R_COL('code','Kode',r=>r.code),R_COL('name',t('common.customer'),r=>r.name),R_COL('end','Berakhir',r=>fdate(r.contractEndDate),{sortv:r=>r.contractEndDate})]},
 {key:'partner_priority_regions',label:t('reports.partner_priority_regions'),need:()=>can('partners','r'),f:[],
  rows:()=>[...new Set(DB.all('customers').map(c=>c.city))].filter(Boolean).map(p=>Partners.regionPriority(p)).filter(r=>r.level!=='Low'),
  cols:[R_COL('p',t('partner.province'),r=>r.province),R_COL('lvl',t('partner.col_priority'),r=>r.level),R_COL('c',t('partner.col_customers'),r=>String(r.custCount),{num:true}),
